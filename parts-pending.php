...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['part_id'], $_POST['action'])) {
    ...
    if ($partId > 0 && in_array($action, ['approve','reject'], true)) {
    ...
    header('Location: parts-pending.php');
    exit;
}

// Load pending parts
$stmt = $db->query("
    SELECT spare_part_id, name, category, price, seller_id, created_at
    FROM spare_parts
...
<head>
    <meta charset="UTF-8">
    <title>Pending Spare Parts - Admin</title>
    ...
    <style>
        ...
        .card{background:#020617;border-radius:0.75rem;padding:1rem 1.1rem;border:1px solid #111827;}
...
<div class="layout">
    ...
    <main class="main">
        <h1 class="page-title">Pending spare parts</h1>
        <p class="page-subtitle">Approve or reject spare part listings before they appear publicly.</p>

        <section class="card">
            <?php if (empty($parts)): ?>
                <p style="color:#9ca3af;">No pending spare parts at the moment.</p>
            <?php else: ?>
                <table>
                    <thead>
                    <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <form method="post" style="display:inline;">
                                    <input type="hidden" name="part_id" value="<?php echo (int)$p['spare_part_id']; ?>">
